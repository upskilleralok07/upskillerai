import { Trophy, Crown, Swords, ChevronRight, TrendingUp, Medal } from 'lucide-react';
...
const defaultEntries: LeaderboardEntry[] = [
  ...
  { rank: 8, name: "Deepika Roy", xp: 2750, problemsSolved: 82, streak: 10 },
];

const BatchLeaderboard = ({
  entries = defaultEntries,
  currentUserId
}: BatchLeaderboardProps) => {
      ...
      <CardContent>
        <Tabs defaultValue="xp" className="w-full">
          ...
          <TabsContent value="problems" className="space-y-2">
            {[...entries].sort((a, b) => b.problemsSolved - a.problemsSolved).map((entry, index) => (
              <div 
                key={entry.rank}
                className="flex items-center gap-3 p-3 rounded-xl transition-all hover:bg-muted/50"
              >
          ...
          <TabsContent value="streak" className="space-y-2">
            {[...entries].sort((a, b) => b.streak - a.streak).map((entry, index) => (
              <div 
                key={entry.rank}
                className="flex items-center gap-3 p-3 rounded-xl transition-all hover:bg-muted/50"
              >
